...
function BookDiaryDetail() {
	...
	const getMonthNum = async () => {
		const getId = await axios.get(`https://api.doranssam.com/diaries?year=${year}&month=${month}`, {
			headers: {
		...
		const getDetail = await axios.get(`https://api.doranssam.com/diaries/book?year=${year}&month=${month}`, {
			headers: {
				Authorization: `Bearer ${cookies['accessToken']}`,
				'Content-type': 'application/json',
			},
							...
							<div className="diarycontents">
								<div className="contents-box">
									<div className="answers">
										...
										{!correct ? (
											<div className="answer">{data.original_text}</div>
										) : (
												...
												{data.correct_text &&
													data.correct_text.map((text) =>
														text[0] === '#' ? (
															...
															<span className="answer_black">{text}</span>
														),
													)}
											</div>
										)}
